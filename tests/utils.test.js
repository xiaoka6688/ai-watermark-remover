...
describe('formatFilename', () => {
  ...
  test('默认模板生成正确文件名', () => {
    ...
    expect(result).toMatch(/^doubao_20260701_01\.mp4$/);
  });
  ...
  test('带标题的模板', () => {
    ...
    expect(result).toMatch(/^dreamina_可爱猫咪_20260701_03\.mp4$/);
  });
  ...
  test('特殊字符被替换为下划线', () => {
    ...
    // 尾部下划线被清理（_\. → .）
    expect(result).toMatch(/^test_a_b_c_d\.png$/);
  });
  ...
  test('自定义模板变量替换', () => {
    ...
    expect(result).toBe('v123_xyq.mp4');
  });
