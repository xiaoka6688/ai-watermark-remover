...
(function() {
  ...
  function injectStyles() {
    if (document.getElementById('aiwm-qw-styles')) return;
    const style = document.createElement('style');
    style.id = 'aiwm-qw-styles';
    style.textContent = `
      .aiwm-dl-btn {
        position: absolute;
        bottom: 10px;
      ...
      *:hover > .aiwm-dl-btn {
      ...
      .aiwm-dl-btn:disabled {
        opacity: 1;
        pointer-events: none;
        cursor: not-allowed;
      }
      .aiwm-dl-btn.aiwm-error {
  ...
  async function downloadFile(url, filename, btn, isImage = false) {
    try {
      ...
      if (btn) {
        ...
        btn.classList.add('aiwm-error');
        setTimeout(() => {
          btn.innerHTML = isImage ? `${DOWNLOAD_ICON} 下载原图` : `${DOWNLOAD_ICON} 下载视频`;
          btn.classList.remove('aiwm-error');
          btn.disabled = false;
        }, 2000);
      }
      return false;
  ...
  function injectImageButton(imgElement) {
    ...
    if (!container) return;
    if (container.querySelector('.aiwm-dl-btn')) return;
  ...
  function startObserver() {
    const observer = new MutationObserver(() => {
      const images = document.querySelectorAll('img:not([data-aiwm-processed])');
      for (const img of images) {
        img.setAttribute('data-aiwm-processed', 'true');
