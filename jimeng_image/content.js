...
(function() {
  ...
  function showToast(message, isError = false) {
    const existing = document.querySelector('.aiwm-jm-toast');
    if (existing) existing.remove();

    const toast = document.createElement('div');
    toast.className = 'aiwm-jm-toast';
  ...
  // 监听 dreamina 模块发来的视频数据
  window.addEventListener('message', function(event) {
    if (event.source !== window || !event.data) return;
    if (event.data.source === 'dreamina-interceptor' && event.data.type === 'VIDEO_INTERCEPTED') {
      videoCount++;
  ...
  function addButtonToImage(img) {
    if (img.hasAttribute('data-aiwm-jm-processed')) return;

    const parent = img.parentNode;
    if (!parent) return;

    // 避免嵌套包装
    if (parent.classList && parent.classList.contains('aiwm-jm-img-wrapper')) return;
    ...
    wrapper.className = 'aiwm-jm-img-wrapper';
    ...
    img.setAttribute('data-aiwm-jm-processed', 'true');
  ...
  function scanAndAddAllButtons() {
    ...
    for (const img of images) {
      if (img.hasAttribute('data-aiwm-jm-processed')) continue;
  ...
  function createFloatingPanel() {
    ...
    floatingPanel.id = 'aiwm-jm-panel';
    ...
    floatingPanel.innerHTML = `
      <div class="aiwm-jm-header" style="
        padding: 12px 16px;
      ...
      ">
        <span style="font-size: 14px; font-weight: 600; color: #5c3d2e;">🎨 即梦图片</span>
        <button class="aiwm-jm-minimize" style="
          background: none;
          border: none;
          color: #8b6b4f;
          cursor: pointer;
      ...
      <div class="aiwm-jm-content" style="padding: 16px;">
        <div class="aiwm-jm-stats" style="
          display: flex;
          align-items: center;
        ...
        ">
          <div style="text-align: center;">
            <div style="font-size: 24px; font-weight: 700; color: #5c3d2e;" class="aiwm-jm-vid-count">0</div>
            <div style="font-size: 11px; color: #8b6b4f;">已检测视频</div>
          </div>
          <div style="width:1px;height:30px;background:rgba(0,0,0,0.1);"></div>
          <div style="text-align: center;">
            <div style="font-size: 24px; font-weight: 700; color: #5c3d2e;" class="aiwm-jm-img-count">0</div>
            <div style="font-size: 11px; color: #8b6b4f;">已下载图片</div>
          </div>
          <div style="width:1px;height:30px;background:rgba(0,0,0,0.1);"></div>
          <div style="text-align: center;">
            <div style="font-size: 24px; font-weight: 700; color: #5c3d2e;" class="aiwm-jm-found-count">0</div>
    ...
    // 拖拽
    const header = floatingPanel.querySelector('.aiwm-jm-header');
    header.addEventListener('mousedown', (e) => {
      if (e.target.classList.contains('aiwm-jm-minimize')) return;
      isDragging = true;
    ...
    // 最小化
    const minimizeBtn = floatingPanel.querySelector('.aiwm-jm-minimize');
    const content = floatingPanel.querySelector('.aiwm-jm-content');
  ...
  function updateFloatingPanelUI() {
    if (!floatingPanel) return;
    const imgCount = floatingPanel.querySelector('.aiwm-jm-img-count');
    const vidCount = floatingPanel.querySelector('.aiwm-jm-vid-count');
    const foundCount = floatingPanel.querySelector('.aiwm-jm-found-count');
    const found = document.querySelectorAll('img[data-aiwm-jm-processed="true"]').length;
    if (imgCount) imgCount.textContent = downloadStats.images;
    if (vidCount) vidCount.textContent = videoCount;
  ...
  function init() {
    ...
    style.textContent = `
      ...
      .aiwm-jm-img-wrapper:hover button {
        opacity: 1 !important;
      }
    `;
