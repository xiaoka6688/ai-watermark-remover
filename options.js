...
(function () {
  ...
  const siteCheckboxes = {
    ...
    klingai: document.getElementById('site_klingai'),
    vidu: document.getElementById('site_vidu'),
    pixverse: document.getElementById('site_pixverse')
  };
  ...
  const archiveInput = document.getElementById('archive_prefix');
  const DEFAULT_TEMPLATE = '{site}_{date}_{idx}.{ext}';
  const DEFAULT_ARCHIVE = 'AI去水印';
  ...
  function loadTemplate() {
    ...
    chrome.runtime.sendMessage({ type: 'GET_ARCHIVE_PREFIX' }, (resp) => {
      if (resp?.success) {
        archiveInput.value = resp.prefix || DEFAULT_ARCHIVE;
      }
    });
  }
  ...
  document.getElementById('btn_save_template').addEventListener('click', () => {
    ...
    const prefix = archiveInput.value.trim();
    chrome.runtime.sendMessage({ type: 'SET_ARCHIVE_PREFIX', prefix: prefix || DEFAULT_ARCHIVE }, (resp) => {
      if (resp?.success) showToast('✅ 归档目录已保存', 'success');
    });
  });

  document.getElementById('btn_reset_template').addEventListener('click', () => {
    templateInput.value = DEFAULT_TEMPLATE;
    archiveInput.value = DEFAULT_ARCHIVE;
    chrome.runtime.sendMessage({ type: 'SET_FILENAME_TEMPLATE', template: DEFAULT_TEMPLATE });
    chrome.runtime.sendMessage({ type: 'SET_ARCHIVE_PREFIX', prefix: DEFAULT_ARCHIVE }, (resp) => {
      if (resp?.success) showToast('已恢复默认设置', 'info');
    });
  });
  ...
  document.getElementById('btn_reset_all').addEventListener('click', () => {
    ...
    chrome.storage.local.remove([
      'aiwm_d15_enabled', 'codex_doubao_video_duration_choice',
  ...
  function applyTheme(theme) {
    if (theme === 'dark') {
      ...
      // auto: 跟随系统，移除 data-theme 让 CSS 媒体查询生效
      document.documentElement.removeAttribute('data-theme');
    }
  }
  ...
  themeSelect.addEventListener('change', () => {
    ...
    applyTheme(theme);
    showToast(`主题已切换为：${theme === 'auto' ? '跟随系统' : theme === 'dark' ? '深色' : '浅色'}`, 'success');
  });
